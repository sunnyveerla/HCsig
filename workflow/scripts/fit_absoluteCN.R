...
ploidy <- as.numeric(sample_df[which(sample_df$sample == paste0(sample_ID,'.sorted')), 'ploidy'])
cellularity <- as.numeric(sample_df[which(sample_df$sample == paste0(sample_ID,'.sorted')), 'cellularity'])
...
sample_copy_number <- copy_number_for_sample(copy_number, sample = paste0(sample_ID,'.sorted'))
